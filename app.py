...
@app.route('/admin/download_tomorrow_excel')
@login_required
def download_tomorrow_excel():
    ...
    try:
        ...
        # ✅ Students who want mess TOMORROW (after mess cut)
        query = """
            SELECT
                u.name,
                u.course,
                u.phone,
                COALESCE(u.delivery_address, 'Hostel') AS address,
                u.meal_type,
                CASE
                    WHEN u.delivery_enabled = 1 THEN 'Delivery'
                    ELSE 'Non-Delivery'
        ...
        return send_file(
            output,
            as_attachment=True,
            download_name=f"tomorrow_mess_list_{tomorrow}.xlsx",
